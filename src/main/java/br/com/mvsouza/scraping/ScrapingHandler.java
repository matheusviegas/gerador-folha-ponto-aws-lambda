...
public class ScrapingHandler {
    ...
    public ABSGPResponseWrapper execute() throws ScrapingException, InvalidCredentialsException {
        try {
            driver.get(Constants.URL_LOGIN_PAGE);

            WebElement txtLogin = driver.findElement(By.cssSelector(Constants.EMAIL_INPUT_CSS_SELECTOR));
            txtLogin.sendKeys(scrapingInfo.getEmail());

            WebElement txtSenha = driver.findElement(By.cssSelector(Constants.PASSWORD_INPUT_CSS_SELECTOR));
            txtSenha.sendKeys(scrapingInfo.getPassword());

            driver.findElement(By.cssSelector(Constants.LOGIN_BUTTON_CSS_SELECTOR)).click();

            this.validateCredentials(driver);

            driver.get(GeneralHelpers.buildJsonDownloadURL(scrapingInfo.getDateReference()));
            String jsonContent = driver.findElement(By.cssSelector(Constants.JSON_CONTENT_CSS_SELECTOR)).getText();
    ...
    private void validateCredentials(WebDriver driver) throws InvalidCredentialsException {
        List<WebElement> alertBox = driver.findElements(By.cssSelector(Constants.ALERT_BOX_CSS_SELECTOR));
